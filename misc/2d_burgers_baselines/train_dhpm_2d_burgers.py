...
class Burgers_DHPM:
    ...
    def __init__(self, Collo, UP, LW, RT, LF, uv_layers, f_layers, lb, ub, uvDir=None, fDir=None):
        ...
        self.u_c_tf = tf.placeholder(tf.float64, shape=[None, self.u_c.shape[1]])
        self.v_c_tf = tf.placeholder(tf.float64, shape=[None, self.v_c.shape[1]])
        ...
        self.loss = 0.1*self.loss_f_uv + 4000*self.loss_uv + 5*self.loss_PBC
    ...
    def sine_init(self, size):
        in_dim = size[0]
        out_dim = size[1]
        w = tf.random_uniform([in_dim, out_dim], minval=-np.sqrt(6/in_dim)/1, maxval=np.sqrt(6/in_dim)/1, dtype=tf.float64)
        return tf.Variable(w, dtype=tf.float64)
    ...
    def net_f(self, x, y, t):
        ...
        N_uv = self.neural_net_f(tf.concat([lap_u, lap_v, u*ux, v*uy, u*vx, v*vy], 1), self.f_weights, self.f_biases)
...
def postProcess(xmin, xmax, ymin, ymax, field=[], s=12, num=0):
    ...
    ######
    cf = ax[0, 0].scatter(x_pred, y_pred, c=u_pred, alpha=0.9, edgecolors='none', cmap='RdYlBu', marker='s', s=s, vmin=-2, vmax=1.6)
    ax[0, 0].axis('square')
    ax[0, 0].set_xticks([])
    ax[0, 0].set_yticks([])
    ...
    ##########
    cf = ax[1, 0].scatter(x_pred, y_pred, c=u_trth, alpha=0.9, edgecolors='none', cmap='RdYlBu', marker='s', s=s, vmin=-2, vmax=1.6)
    ax[1, 0].axis('square')
    ax[1, 0].set_xticks([])
    ax[1, 0].set_yticks([])
    ...
    cbar.ax.tick_params(labelsize=18)
    #######
    cf = ax[0, 1].scatter(x_pred, y_pred, c=v_pred, alpha=0.9, edgecolors='none', cmap='RdYlBu', marker='s', s=s, vmin=-2.8, vmax=0.5)
    ax[0, 1].axis('square')
    ax[0, 1].set_xticks([])
    ax[0, 1].set_yticks([])
    ...
    cbar.ax.tick_params(labelsize=18)
    #####
    cf = ax[1, 1].scatter(x_star, y_star, c=v_trth, alpha=0.9, edgecolors='none', cmap='RdYlBu', marker='s', s=s, vmin=-2.8, vmax=0.5)
    ax[1, 1].axis('square')
    ax[1, 1].set_xticks([])
    ax[1, 1].set_yticks([])
    ...
    cbar.ax.tick_params(labelsize=18)
    #####
    plt.savefig('./output/uv_comparison_'+str(num).zfill(4)+'.png',dpi=100)
    plt.close('all')

if __name__ == "__main__":

    # Domain bounds
    MAX_T = 0.1                         # dt=0.00025 in PeRCNN (400 steps)
    lb = np.array([ 0,   0,    0])
    ub = np.array([ 1,   1,  0.1])      # fix the scaler for pretraining and fietuning

    # Network configuration
    uv_layers = [3] + 5 * [120] + [2]
    f_layers  = [6] + 2 * [20] + [2]

    # add initial regularizer ?

    # Initial condition point for u, v
    x, y = [np.linspace(0, 1, 101)]*2
    x, y = np.meshgrid(x[::2], y[::2])
    x, y = x.flatten()[:, None], y.flatten()[:, None]
    truth = scipy.io.loadmat('../../2d_burgers/data/Burgers_2001x2x100x100_[dt=00025].mat')['uv']
    ...
    UV = UV[:, 100:501:40, ::2, ::2]                                        # Downsampling
    XYT_c = []
    for i in range(UV.shape[1]):
        t = 0.1/(UV.shape[1]-1)*i
    ...
    LW = [0, 0, 0] + [1.0, 0, MAX_T] * lhs(3, 8000)
    UP = np.concatenate((LW[:, 0:1], LW[:, 1:2]+1.0, LW[:, 2:3]), axis=1)

    LF = [0, 0, 0] + [0, 1.0, MAX_T] * lhs(3, 8000)
    ...
    with tf.device('/device:GPU:1'):
        ...
        # train from scrtach
        # model = Burgers_DHPM(XYT_c, UP, LW, RT, LF, uv_layers, f_layers, lb, ub)
        # load trained model
        model = Burgers_DHPM(XYT_c, UP, LW, RT, LF, uv_layers, f_layers, lb, ub,
                             uvDir='uv_NN_5x120_float64.pickle', fDir='f_NN_2x20_float64.pickle')

        model.getloss()

        start_time = time.time()
        model.train(iter=5000, learning_rate=4e-3)
        model.train_bfgs()
        print("--- %s seconds ---" % (time.time() - start_time))

        ################End of pre-train####################

        model.save_NN_f('f_NN_2x20_float64.pickle')
        model.save_NN_uv('uv_NN_5x120_float64.pickle')

        model.getloss()

        N_t = 41
        # Output result at each time step
        x_star = np.linspace(0, 1, 101)
        y_star = np.linspace(0, 1, 101)
        ...
        for i in range(N_t):
            ...
            u_trth, v_trth = truth[0, 100+10*i, :, :].flatten()[:, None],  truth[1, 100+10*i, :, :].flatten()[:, None]       # 2x100x100
            field = [x_star, y_star, t_star, u_pred, v_pred, u_trth, v_trth]
            postProcess(xmin=0, xmax=1, ymin=0, ymax=1, s=6, field=field, num=i)

        # save results
        UV = []
        x_star, y_star = [np.linspace(0, 1, 101)] * 2
        x_star, y_star = np.meshgrid(x_star[:-1], y_star[:-1])
        x_star, y_star = x_star.flatten()[:, None], y_star.flatten()[:, None]
        for i in range(1601):
            print('num=', i)
            t_star = 0.00025*i + 0*x_star
            u_pred, v_pred = model.predict(x_star, y_star, t_star)
        ...
        scipy.io.savemat('uv_2x1601x100x100_[DHPM].mat', {'uv': UV[:, :, :, :]})
