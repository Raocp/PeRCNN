...
class convlstm(nn.Module):
    def __init__(self, input_channels, hidden_channels, 
        ...
        # self.dx, self.dt, self.nu_up = 1/100, 0.00025, 0.01  # upper bound of nu
        # np.random.seed(1234)
...
def train(model, input, initial_state, truth, u0_obs, n_iters, lr, save_path, cont=True):
    ...
    for epoch in range(n_iters):
        ...
        # save model
        if epoch%10==0 and data_loss < best_loss:
...
if __name__ == '__main__':
    ...
    start = time.time()
    train_loss, val_loss = train(model, input, initial_state, truth, u0_obs, n_iters, lr, model_save_path, cont=False)
    end = time.time()
    ...
    scipy.io.savemat('uv_2x1601x100x100_[ConvLSTM].mat', {'uv': UV[:,:,:,:]})
