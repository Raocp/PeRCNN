...
class upscaler(nn.Module):
    ...
    def __init__(self):
        ...
        self.layers = []
        self.conv0 = nn.ConvTranspose2d(2, 16, kernel_size=5, padding=5 // 2, stride=2, output_padding=1, bias=True)
        self.conv1 = nn.Conv2d(16, 16, 5, stride=1, padding=5//2, padding_mode='circular', bias=True)
        self.tanh = torch.nn.Tanh()
        # self.up1 = nn.ConvTranspose2d(8, 8, kernel_size=5, padding=5 // 2, stride=2, output_padding=1, bias=True)
        self.out = nn.Conv2d(16, 2, 1, 1, padding=0, bias=True)
        self.convnet = torch.nn.Sequential(self.conv0, self.tanh, self.conv1, self.tanh, self.out)
...
class RCNNCell(nn.Module):
    ...
    def __init__(self, input_channels, hidden_channels, output_channels, input_kernel_size,
        ...
        self.Wh3_u = nn.Conv2d(in_channels=2, out_channels=16, kernel_size=5,
                               stride=self.input_stride, padding=0, bias=True, )
        self.Wh1x1_u = nn.Conv2d(in_channels=16, out_channels=1, kernel_size=1,
                                 stride=1, padding=0, bias=True)
        # Nonlinear term for v (up to 2nd order)
        ...
        self.Wh3_v = nn.Conv2d(in_channels=2, out_channels=16, kernel_size=5,
                               stride=self.input_stride, padding=0, bias=True, )
        self.Wh1x1_v = nn.Conv2d(in_channels=16, out_channels=1, kernel_size=1,
                               stride=1, padding=0, bias=True)
        ...
        self.init_filter(self.filter_list, c=0.05)
    ...
    def init_filter(self, filter_list, c):
        ...
        for filter in filter_list:
            ...
            # filter.weight.data.uniform_(-c * np.sqrt(1 / (5 * 5 * 8)), c * np.sqrt(1 / (5 * 5 * 8)))
...
def train(model, truth, n_iters, time_batch_size, learning_rate, dt, dx, restart=True): # restart=False
    ...
    for epoch in range(n_iters):
        ...
        optimizer.zero_grad()
        num_time_batch = 1
        ic_loss, batch_loss, phy_loss, data_loss, val_loss = [0]*5
        for time_batch_id in range(num_time_batch):
            ...
            loss = 1*loss_data + 1.0*loss_ic  # IC regularizer is used when goes with ISG
            loss.backward(retain_graph=True)
            batch_loss += loss.item()
            ic_loss, phy_loss, data_loss, val_loss = loss_ic.item(), loss_phy.item(), loss_data.item(), loss_valid.item()
        optimizer.step()
        scheduler.step()
        # print loss in each epoch
        print('[%d/%d %d%%] loss: %.7f, data_loss: %.7f, val_loss: %.7f, ic_loss%.5f, phy_loss: %.7f' % ((epoch+1), n_iters, ((epoch+1)/n_iters*100.0),
              batch_loss, data_loss, val_loss, ic_loss, phy_loss))
...
if __name__ == '__main__':
    ...
    # train the model
    start = time.time()
    # pretrain_upscaler(model.UpconvBlock, init_state_low, epoch=4000, plotFlag=True) # not used since IC is interpolated
    ...
    scipy.io.savemat('uv_2x1601x100x100_[PeRCNN,withISG,Diff,lam100].mat', {'uv': UV[:,:,:-1,:-1]})
