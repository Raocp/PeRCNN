...
class RCNNCell(nn.Module):
    ...
    def __init__(self, input_channels, hidden_channels, input_kernel_size, output_channels=1):
        ...
        self.Wh1_u = nn.Conv2d(in_channels=input_channels, out_channels=hidden_channels, kernel_size=5,
                               stride=self.input_stride, padding=0, bias=True, )
        self.Wh2_u = nn.Conv2d(in_channels=input_channels, out_channels=hidden_channels, kernel_size=5,
                               stride=self.input_stride, padding=0, bias=True, )
        self.Wh3_u = nn.Conv2d(in_channels=input_channels, out_channels=hidden_channels, kernel_size=5,
                               stride=self.input_stride, padding=0, bias=True, )
        self.Wh1x1_u = nn.Conv2d(in_channels=hidden_channels, out_channels=1, kernel_size=1,
                                 stride=1, padding=0, bias=True)
        # Nonlinear term for v (up to 3rd order)
        self.Wh1_v = nn.Conv2d(in_channels=input_channels, out_channels=hidden_channels, kernel_size=5,
                               stride=self.input_stride, padding=0, bias=True, )
        self.Wh2_v = nn.Conv2d(in_channels=input_channels, out_channels=hidden_channels, kernel_size=5,
                               stride=self.input_stride, padding=0, bias=True, )
        self.Wh3_v = nn.Conv2d(in_channels=input_channels, out_channels=hidden_channels, kernel_size=5,
                               stride=self.input_stride, padding=0, bias=True, )
    ...
    def forward(self, h):
        ...
        u_res = self.Wh1x1_u( self.Wh1_u(h_pad_2)*self.Wh2_u(h_pad_2)*self.Wh3_u(h_pad_2) )
        v_res = self.Wh1x1_v( self.Wh1_v(h_pad_2)*self.Wh2_v(h_pad_2)*self.Wh3_v(h_pad_2) )
...
def load_model(model):
    ...
    checkpoint = torch.load('./model/checkpoint_wo_diff.pt')
...
def postProcess_v2(output, truth, low_res, xmin, xmax, ymin, ymax, num, fig_save_path):
    ...
    #
    plt.savefig(fig_save_path + 'uv_comparison_'+str(num).zfill(4)+'.png')
    plt.close('all')
...
if __name__ == '__main__':
    ...
    # train the model
    start = time.time()
    # pretrain_upscaler(model.UpconvBlock, init_state_low, epoch=8000, plotFlag=True) # not used since IC is interpolated
    # Set cont=True to continue training
