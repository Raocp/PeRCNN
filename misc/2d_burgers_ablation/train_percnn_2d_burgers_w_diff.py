...
class RCNNCell(nn.Module):
    ...
    def __init__(self, input_channels, hidden_channels, input_kernel_size, output_channels=1):
        super(RCNNCell, self).__init__()

        # the initial parameters
        self.input_channels = input_channels
        self.hidden_channels = hidden_channels
        self.output_channels = output_channels  # useless, always 1
        self.input_kernel_size = input_kernel_size
        self.input_stride = 1

        self.dx = 1/100
        self.dt = 0.00025
        self.nu_up = 0.0103  # nu from 0 to upper bound (two times the estimate)
        np.random.seed(1234)
        ...
        self.Laplace = nn.Conv2d(1, 1, self.input_kernel_size, self.input_stride, padding=0, bias=False)
        self.Laplace.weight.data = 1/self.dx**2*torch.tensor(lap_2d_op, dtype=torch.float32)
        self.Laplace.weight.requires_grad = False

        # Nonlinear term for u (up to 3rd order)
        self.Wh1_u = nn.Conv2d(in_channels=2, out_channels=hidden_channels, kernel_size=input_kernel_size,
                               stride=self.input_stride, padding=0, bias=True, )
        self.Wh2_u = nn.Conv2d(in_channels=2, out_channels=hidden_channels, kernel_size=input_kernel_size,
                               stride=self.input_stride, padding=0, bias=True, )
        self.Wh3_u = nn.Conv2d(in_channels=2, out_channels=hidden_channels, kernel_size=input_kernel_size,
                               stride=self.input_stride, padding=0, bias=True, )
        self.Wh1x1_u = nn.Conv2d(in_channels=hidden_channels, out_channels=1, kernel_size=1,
                                 stride=1, padding=0, bias=True)
        # Nonlinear term for v (up to 3rd order)
        self.Wh1_v = nn.Conv2d(in_channels=2, out_channels=hidden_channels, kernel_size=input_kernel_size,
                               stride=1, padding=0, bias=True, )
        self.Wh2_v = nn.Conv2d(in_channels=2, out_channels=hidden_channels, kernel_size=input_kernel_size,
                               stride=1, padding=0, bias=True, )
        self.Wh3_v = nn.Conv2d(in_channels=2, out_channels=hidden_channels, kernel_size=input_kernel_size,
                               stride=1, padding=0, bias=True, )
        self.Wh1x1_v = nn.Conv2d(in_channels=hidden_channels, out_channels=1, kernel_size=1,
                               stride=1, padding=0, bias=True)

        # initialize filter's wweight and bias
        self.filter_list = [self.Wh1_u, self.Wh2_u, self.Wh3_u, self.Wh1x1_u, self.Wh1_v, self.Wh2_v, self.Wh3_v, self.Wh1x1_v]
        self.init_filter(self.filter_list, c=0.1)
    ...
    def forward(self, h):
        ...
        u_res = self.nu_up*torch.sigmoid(self.Cu)*self.Laplace(u_pad_2) + self.Wh1x1_u( self.Wh1_u(h_pad_2)*self.Wh2_u(h_pad_2)*self.Wh3_u(h_pad_2) )
        v_res = self.nu_up*torch.sigmoid(self.Cv)*self.Laplace(v_pad_2) + self.Wh1x1_v( self.Wh1_v(h_pad_2)*self.Wh2_v(h_pad_2)*self.Wh3_v(h_pad_2) )
...
class RCNN(nn.Module):
    ...
    def forward(self):
        ...
        # To eliminate the effects of ISG, we use interpolated IC
...
def train(model, truth, n_iters, time_batch_size, learning_rate, dt, dx, cont=True):
    ...
    for epoch in range(n_iters):
        ...
        optimizer.zero_grad()
        num_time_batch = 1
        batch_loss, phy_loss, data_loss, val_loss = [0]*4
        for time_batch_id in range(num_time_batch):
            ...
            # get physics loss (for validation, not used for training or model selection)
            loss_phy = loss_gen(output, loss_func)
            loss = 1*loss_data     # + 2.0*loss_ic  # IC is fixed in this test
            loss.backward(retain_graph=True)
...
def load_model(model):
    ...
    checkpoint = torch.load('./model/checkpoint_w_diff.pt')
...
if __name__ == '__main__':
    ...
    n_iters = 10000   # 20000 recommended
    learning_rate = 2e-3
    save_path = './model/'
    ...
    start = time.time()
    # To eliminate the effects of ISG, we use interpolated IC
    # pretrain_upscaler(model.UpconvBlock, init_state_low, epoch=6000)
    # Set cont=False to train from scratch
    train_loss_list = train(model, truth, n_iters, time_batch_size, learning_rate, dt, dx, cont=True)
    end = time.time()
