...
class ResidualBlock(nn.Module):
    def __init__( self, inplanes: int, planes: int, stride: int = 1,  norm_layer = None) -> None:
        ...
        np.random.seed(1234)
        self.Cu = torch.nn.Parameter(torch.tensor((np.random.rand()-1)*2, dtype=torch.float32), requires_grad=True)
        self.Cv = torch.nn.Parameter(torch.tensor((np.random.rand()-1)*2, dtype=torch.float32), requires_grad=True)

        self.Laplace = nn.Conv2d(1, 1, 5, stride, padding=5//2, padding_mode='circular', bias=False)
        ...
        self.filter_list = [self.conv1, self.conv2, self.conv3]
        self.init_filter(self.filter_list, c=0.2)
    ...
    def init_filter(self, filter_list, c):
        ...
        for filter in filter_list:
            N = np.prod(filter.weight.shape)
            filter.weight.data.uniform_(-c * np.sqrt(1 / N), c * np.sqrt(1 / N))
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # We incorporate diffusion connection for ablation study
        laplace_u = self.Laplace(x[:, 0:1, ...])*self.nu_up*torch.sigmoid(self.Cu)
        laplace_v = self.Laplace(x[:, 1:2, ...])*self.nu_up*torch.sigmoid(self.Cv)
        laplace_x = torch.cat((laplace_u, laplace_v), dim=1)
        ...
        out = out + identity + laplace_x*self.dt
...
class RecurrentResNet(nn.Module):
    ...
    def __init__(self, channels, init_state_low, input_kernel_size,
...
if __name__ == '__main__':
    ...
    model = RecurrentResNet(channels=None,
    ...
    scipy.io.savemat('uv_2x1601x100x100_[ResNet,Diff].mat', {'uv': UV[:, :-1, :-1, :-1]})
